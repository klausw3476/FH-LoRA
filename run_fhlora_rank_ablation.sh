#!/bin/bash
# =============================================================================
# FH-LoRA — LoRA rank sweep on SSv2 (Phase 2 distill + linear probe).
#
# Tests how SSv2 linear-probe accuracy scales with --lora_rank for the
# proposed FH-LoRA module (--lora_type fh_lora).
# Hyper trunk width is fixed via HYPER_HHD for a fair rank comparison.
#
# Usage:
#   ./run_fhlora_rank_ablation.sh
#   RANKS="8 16 32" ./run_fhlora_rank_ablation.sh
#   EVAL_ONLY=1 ./run_fhlora_rank_ablation.sh   # eval only (checkpoints exist)
#
...
for RANK in $RANKS; do
    TAG="fhlora_r${RANK}_hhd${HYPER_HHD}"
    PHASE2="output_ssv2/phase2_${TAG}"
    EVAL="output_ssv2/eval_${TAG}"

    echo "============================================"
    echo " FH-LoRA  rank=$RANK  hyper_hidden_dim=$HYPER_HHD"
    echo "============================================"
...
echo ""
echo "Done. Compare best test_acc1 in each log under output_ssv2/eval_fhlora_r*_hhd${HYPER_HHD}/"
