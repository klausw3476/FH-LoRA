...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W, return_attn=False, capture_temporal_attn=None):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            if capture_temporal_attn is not None:
                temp_out, temp_attn = self.temporal_attn(
                    xt_norm, return_attn=True)
                # (BHW, heads, T, T) -> (BHW, T, T); matches distill hook aggregation
                capture_temporal_attn.append(temp_attn.mean(dim=1))
                res_temporal = self.drop_path(temp_out)
            else:
                res_temporal = self.drop_path(self.temporal_attn(xt_norm))
